 ...
 ******************************************************************** 
 * 
 * com.duoyu001.framework.game.utils.SelectorUtils.java CREATED ON 下午3:41:42
 ...
 * @date 2014年10月21日 下午3:41:42
 */
public class SelectorUtils {

	public static <T> List<T> selector(	Collection<T> collections,
										ISelector<T> selector) {
		List<T> list = new LinkedList<T>();
		for (T t : collections) {
			if (selector.isNeedSelect()) {
				T select = selector.select(t);
				if (select != null) {
					list.add(select);
				}
			} else {
	 ...
	 * @return
	 */
	public static <T, E> List<T> selector2Mslx(	Map<E, T> m,
												ISelector<T> selector) {
		List<T> tList = new LinkedList<T>();
		for (T ts : m.values()) {

			if (selector.isNeedSelect()) {
				T select = selector.select(ts);
				if (select != null) {
					tList.add(select);
				}
			} else {
