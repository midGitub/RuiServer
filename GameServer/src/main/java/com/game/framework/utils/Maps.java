 ...
 ******************************************************************** 
 * 
 * com.duoyu001.xgame.modules.resource.ResourceHelps.java CREATED ON 上午10:35:39
...
public class Maps {
	 ...
	 * @param map2
	 */
	public static <T> void add(Map<T, Integer> map1, Map<T, Integer> map2) {
		for (Entry<T, Integer> entry : map2.entrySet()) {
			Integer integer = map1.get(entry.getKey());
			if (integer == null) {
				map1.put(entry.getKey(), entry.getValue());
			} else {
				map1.put(entry.getKey(), entry.getValue() + integer);
			}
		}
	}

	/**
	 * map1的value 减去 map2的value
	 * 
	 * @param map1
	 * @param map2
	 */
	public static <T> void minus(Map<T, Integer> map1, Map<T, Integer> map2) {
		for (Entry<T, Integer> entry : map2.entrySet()) {
			Integer integer = map1.get(entry.getKey());
			if (integer == null) {
				map1.put(entry.getKey(), -entry.getValue());
